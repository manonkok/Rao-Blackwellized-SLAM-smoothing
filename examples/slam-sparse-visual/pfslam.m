function [traj_mean,m,P,rmse_path,rmse_th,rmse_map] = ...
    pfslam(Y,u,map,groundTruth,initPos,initTheta,initMapVar,noiseVar,guessMapVar,f,fp,fw,visualization,savevideo)
%% PFSLAM - Perform sparse particle filter based simultaneous localization and mapping (SLAM)
%
% Syntax:
...
%
% Description:
%   This function performs SLAM using a particle filter approach. It takes
%   sensor observations, control inputs, and initial conditions to estimate
%   the trajectory of the robot and the map of the environment. The function
%   can also visualize the SLAM process and save it as a video. The output
%   includes the mean trajectory estimate, map estimate, and the associated
%   root mean square errors for the path, orientation, and map.
%
% See also:
%   particleFilter, measurement, calc_rmses
%
% Copyright:
...
%% Call external particle filter
  ...
  % Plotting function
  if visualization
    makePlots = @(xn,xl_max,P,traj_max,yhattraj,xn_traj,traj_mean,xl,foo) ...
  ...
  % Run filter
  [traj_max,traj_mean,xl_max,xl_mean,P_max,P_mean,traj_sample,xn_traj] = particleFilter(dynModel,measModel,odometry,y,...
    x0_nonLin,x0_lin,P0_lin,Q,R,N_P,dt,sparseFeatures,makePlots);
      
  % RMSE
  [rmse_path,rmse_th,rmse_map,Z_traj,Z_map] = calc_rmses(map',reshape(xl_mean,2,[])', ...
          map',reshape(xl_mean,2,[])',groundTruth',traj_mean');

  % Outputs m and P undefined
  m = []; P = [];
